...
func TestJobHTTP_Do(t *testing.T) {
	...
	t.Run("Should: return error port 80", func(t *testing.T) {
		s := NewHttpJob(http.MethodGet, "http://google.ru", map[string]string{}, 0, http.StatusOK, &httpToolsMockError{})
		assert.Equal(t, int32(80), s.Do().GetLogData().Meta.Port)
	})
	t.Run("Should: return error port 80", func(t *testing.T) {
		s := NewHttpJob(http.MethodGet, "https://google.ru", map[string]string{}, 0, http.StatusOK, &httpToolsMockError{})
