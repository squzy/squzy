...
func ExecMysql(config *scheduler_config_storage.DbConfig, dbC DBConnector) CheckError {
	...
	if err != nil {
		return newSqlError(logId, startTime, timestamppb.Now(), apiPb.SchedulerCode_ERROR, mysqlConnectionError.Error(), config.Host, config.Port)
	}
	defer dbC.Close()

	err = dbC.Ping()
	if err != nil {
		return newSqlError(logId, startTime, timestamppb.Now(), apiPb.SchedulerCode_ERROR, mysqlPingError.Error(), config.Host, config.Port)
	}

	return newSqlError(logId, startTime, timestamppb.Now(), apiPb.SchedulerCode_OK, "", config.Host, config.Port)
}
...
func (m DBConnection) Connect(driver string, dataSource string) error {
	if client, err := sql.Open(driver, dataSource); err == nil {
		m.Client = client
		return err
	} else {
		return err
